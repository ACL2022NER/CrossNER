...
class CRF(nn.Module):
    # label size(start and stop)
    def __init__(self, tagset_size, gpu):
        super(CRF, self).__init__()
        print ("build batched crf...")
        self.gpu = gpu

        self.average_words = False
        # 28
    ...
    def neg_log_likelihood_loss(self, feats, mask, tags):
        ...
        # exit(0)
        num_words = torch.sum(mask.detach()).float()
        if self.average_words:
            return (forward_score - gold_score)/int(num_words.item())
        else:
